...
public final class PersonWarehouse {
    ...
    public static Stream<Person> getUniquelyNamedPeopleStartingWith(Character character) {
        ...
        return people.stream().distinct().filter(person ->
                person.name.startsWith(""+character));
    }
    ...
    public static Stream<Stream<String>> getNestedAliases() {
        //people.stream().allMatch(person -> getNestedAliases());
        //return people.stream().forEach(person -> Stream.of(person.getAliases()));
        return people.stream().map(person -> Stream.of(person.aliases));
    }
    ...
    public static List<String> getNames() {
       /* ArrayList<String> names = people.stream().forEach(person -> person. );
        return people.stream().forEach(person -> {
            person.name;
        });
