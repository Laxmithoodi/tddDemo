

public class Array2 {

    /**
     * Given an array of ints, return true if value appears as either the first or last element in the array.
     * The array will be length 1 or more.
     * example : firstLast(6, [1,2,6); // Should return true
     * firstLast(6, [1,2,3]); // Should return false
     */

    public boolean firstLast(Integer value, Integer[] input) {
        if (input[0] == value || input[input.length - 1] == value) {
            return true;
        }
        return false;
    }

    /**
     * Given an array of ints, return true if the array is length 1 or more, and the first element and the last element are equal.
     * example : sameFirstLast([1,2,3]); // Should return false
     * sameFirstLast([1,2,1]); // Should return true
     */
    public boolean sameFirstLast(Integer[] input) {
        if (input.length != 0 && input[0] == input[input.length - 1]) {
            return true;
        }
        return false;
    }
/**
 * Given 2 arrays of ints, a and b, return true if they have the same first element or they have the same last element.
 * Both arrays will be length 1 or more.
 * example : commonEnd([1, 2, 3], [7, 3]); // Should return true
 *           commonEnd([1, 2, 3], [7, 3, 2]); // Should return false
 */

    public boolean commonEnd(Integer[] value, Integer[] input){
        if(value[0] == input[0] || value[value.length-1] == input[input.length-1]){
            return true;
        }
        return  false;
    }

    /**
     * Given an array of ints, return an array with the elements "rotated left" so {1, 2, 3} yields {2, 3, 1}.
     * example : rotateLeft([1, 2, 3]); // Should return [2,3,1]
     *           rotateLeft([5, 11, 9]); // Should return [11,9,5]
     */

    public Integer[] rotateLeft(Integer[] input){
        int k=0;
        Integer[] output = new Integer[input.length];

        for (int i = input.length-1; i >0; i--) {
            output[k] = input[i];
            k = k+1;

        }
        return  output;
        }

/**
 * Given an array of ints, figure out which is element in the array is largest,
 * and set all the other elements to be that value. Return the changed array.
 * example : maxValue([1, 2, 3]); // Should return [3,3,3]
 *           maxValue([5, 11, 9]); // Should return [11,11,11]
 */


    public Integer[] maxValue(Integer[] input){
        int max =0;
         Integer[] newArray = new Integer[input.length];

     for (int i = 0; i < input.length; i++) {
        if(input[i]>max) {
            max= input[i];
        }
    }
     for (int i = 0; i < input.length; i++) {
        newArray[i] = max;
    }

    return newArray;


}
    /**
     * Start with 2 int arrays, a and b, each length 2.
     * Consider the sum of the values in each array.
     * Return the array which has the largest sum. In event of a tie, return a.
     */

    public Integer[] biggerTwo(Integer[] a, Integer[] b) {


        Integer sum1 =0;
        Integer sum2 =0;

        for (int i = 0; i < a.length; i++) {
            sum1 = sum1 + a[i];
            sum2 = sum2 + b[i];
        }
                if(sum1==sum2){
                    return a;
                }
         else if(sum1>sum2) {
              return a;
          }
         return b;

        }

    /**
     * Given an array of ints of odd length, return a new array length 3 containing the elements from the middle of the array.
     * The array length will be at least 3.
     * example : midThree([1, 2, 3, 4, 5]); // Should return [2, 3, 4]
     *           midThree([8, 6, 7, 5, 3, 0, 9]); // Should return [7, 5, 3]
     */
//    int array - odd length
    //3 length middle
    
    public Integer[] middleThree(Integer[] input){

       Integer[] array = new Integer[3];

            int length1 = input.length;
            int middle = length1/2;

            array[0]= input[middle-1];
            array[1]= input[middle];
            array[2] = input[middle+1];

            return array;
        }

/**
 * Given 2 int arrays, a and b, return a new array length 2 containing their middle elements, if length is odd.
 * If the array length is even the sum of the middle 2 elements in the array.
 * example : middleWay([1, 2, 3], [4,5,6,2]); // Should return [2,11]
 *           middleWay([5, 1, 2, 9], [3, 4, 5, 5]); // Should return [3, 9]
 */

    public Integer[] middleWay(Integer[] input1, Integer[] input2) {

        Integer[] array1 = new Integer[2];

        int length1 = input1.length;
        int length2 = input2.length;


        if (length1 % 2 == 0) {//even input2
            array1[0] = input1[(input1.length / 2) - 1] + input1[(input1.length / 2)];

        } else {
            array1[0] = input1[input1.length/2];

        }

        if (length2 % 2 == 0) {//even input2
            array1[1] = input2[(input2.length / 2) - 1] + input2[(input2.length / 2)];

        } else {

            array1[1] = input2[(input2.length / 2)];

        }
        return array1;
    }

    




}

